...
export default async function createPassword(): Promise<void> {
	try {
		...
		const nickname = await y.input({
			...
			validate: (value) => {
				...
				const existing = existingData.find(
					(pwd) => pwd.nickname.toLowerCase() === trimmed.toLowerCase(),
				);
				if (existing) {
		...
		if (passwordChoice === "generate") {
			...
			if (!acceptGenerated) {
				console.log(
					colors.warning("⚠️  Password rejected. Returning to main menu."),
				);
				await delay(1500);
				return;
			}
		...
		console.log(
			`   Password: ${colors.muted("•".repeat(Math.min(password.length, 12)))}`,
		);

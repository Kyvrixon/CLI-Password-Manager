...
export default async function settings(): Promise<void> {
	try {
		...
		ui.space();
		console.log(`   ${colors.primary("Theme:")} ${colors.highlight(currentSettings.theme)}`);
		console.log(`   ${colors.primary("Confirm deletions:")} ${currentSettings.confirmDeletions ? colors.success("Enabled") : colors.error("Disabled")}`);
		console.log(`   ${colors.primary("Show password strength:")} ${currentSettings.showPasswordStrength ? colors.success("Enabled") : colors.error("Disabled")}`);
		ui.space();

		// Simple confirmation to return to main menu
		console.log(colors.success("✅ Settings viewed successfully!"));
		console.log(colors.muted("💡 To modify settings, access them individually from the main menu"));
		ui.space();
...
async function configureTheme(settings: UserSettings): Promise<void> {
	...
	for (const theme of themes) {
		...
		console.log(`${prefix} ${theme.name} ${isActive ? colors.success("(current)") : ""}`);
...
async function viewDetailedSettings(settings: UserSettings): Promise<void> {
	...
	console.log(`${colors.primary("Theme:")} ${colors.highlight(settings.theme)}`);
	console.log(`   ${colors.muted("Controls the visual appearance of the interface")}`);
	ui.space();
	
	console.log(`${colors.primary("Deletion Confirmation:")} ${settings.confirmDeletions ? colors.success("Enabled") : colors.error("Disabled")}`);
	console.log(`   ${colors.muted("When enabled, prompts for confirmation before deleting passwords")}`);
	ui.space();
	
	console.log(`${colors.primary("Password Strength Display:")} ${settings.showPasswordStrength ? colors.success("Enabled") : colors.error("Disabled")}`);
	console.log(`   ${colors.muted("Shows strength indicators and security recommendations")}`);
	ui.space();
...
async function exportSettings(settings: UserSettings): Promise<void> {
	try {
		...
		const exportPath = path.join(os.homedir(), "Desktop", "password-manager-settings.json");

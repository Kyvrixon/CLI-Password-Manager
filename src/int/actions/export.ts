...
export default async function exportVault(): Promise<void> {
	try {
		...
		if (passwordData.length === 0) {
			...
			ui.space();
			console.log(colors.muted("💡 Add some passwords first before creating a backup"));
			await delay(2500);
		...
		// Require mastercode for export
		const mastercode = await y.password({
			message: colors.primary("Enter your master code to export all data (WARNING: this will include all passwords and your mastercode in plain text):"),
		});
		...
		try {
			...
			// Export everything in the database as is, including mastercode and password values
			exportData = {
				version: "1.0.0",
			...
			const content = exportType === "encrypted" ?
			...
			ui.space();
			console.log(colors.warning("⚠️  WARNING: This export could contain ALL your data, including passwords and mastercode! Please keep it safe and do not share it!"));
			ui.space();

			if (exportType === "encrypted") {
				console.log(colors.primary("🔐 Your backup is encrypted with your master code."))
			...
			console.log(`   ${colors.primary("File:")} ${colors.highlight(path.basename(fullPath))}`);
			console.log(`   ${colors.primary("Location:")} ${colors.muted(exportDir)}`);
			console.log(`   ${colors.primary("Type:")} ${colors.text(exportType)}`);
			console.log(`   ${colors.primary("Passwords:")} ${colors.highlight(passwordData.length.toString())}`);
			ui.divider("─", 40, colors.success);
			ui.space();

			if (exportType === "encrypted") {
				console.log(colors.warning("🔐 Important: Keep your backup password safe!"));
				console.log(colors.muted("   Without it, the backup cannot be restored."));
				ui.space();
			}
